      ...
      <ProTable ref="proTableRef" :columns="tableColumns" :tool-button="false" :is-show-search="false"
        ...
        <template #tableHeader>
          <div class="flex mb-6px">
            ...
            <el-input placeholder="请输入关键字搜索" style="height: 32px;" prefix-icon="Search"></el-input>
          </div>
        </template>
...
import { basicProjectRef, queryRuleTjInfoList, addRuleTjInfo, addRuleTjCondition, getRelationTypeMap, getRuleLogicTypeMap, paramTypeMap, updateRuleTjCondition, deleteRuleTjCondition, updateRuleTjInfo, deleteRuleTjInfo } from '@/api/basicInfo/diagnosticKnowledge/basicProjecRules'
...
const getBasicInfo = async () => {
  const { data } = await basicProjectRef(route.query.id)
  // console.log(data);
}
getBasicInfo()

const getTableList = async () => {
  const data = await queryRuleTjInfoList(initParams.value)
